...
const actions = {
  SET_Data({ commit }, payload) {
    console.log(payload)
    db.collection('students').doc(payload.nis).set(payload).then((data) => {
  ...
  UPDATE_Data({dispatch}, payload) {
    // console.log(payload)
    db.collection("students").doc(payload.nis).get().then(function (doc) {
      if (doc && doc.exists) {
        // var data = doc.data();
        // saves the data to 'name'
        db.collection("students").doc(payload.nis).set(payload).then(() => {
          dispatch('GET_Data')
          alert("Edited")
